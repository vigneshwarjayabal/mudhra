    ...
    if user_input or st.session_state.selected_organ:
        ...
        if user_input:
            ...
            if organ_en:
                ...
                response = f"The mudra finger for {organ_en.title()} is Right Hand {finger}."
                image_path = get_image_path(finger.strip())
            elif organ_ta:
                finger = tamil_dict[organ_ta]
                response = f"{organ_ta} உடற்கூறிற்கு உரிய முத்திரை விரல் வலது கை {finger} ஆகும்."
                image_path = get_image_path(finger.strip())
            else:
            ...
            if is_tamil and organ in tamil_dict:
                ...
                response = f"{organ} உடற்கூறிற்கு உரிய முத்திரை விரல் வலது கை {finger} ஆகும்."
                image_path = get_image_path(finger)
            elif not is_tamil and organ in eng_dict:
                finger = eng_dict[organ]
                response = f"The mudra finger for {organ.title()} is Right Hand {finger}."
                image_path = get_image_path(finger)
            else:
                response = "🙏 Sorry, I don't have mudra information for that organ."
